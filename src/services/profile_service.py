...
class ProfileService:
    ...
    async def update_profile(self, profile: Profile) -> None:
        ...
        '''
        try:
            profile_old, profile_new = await self._profile_repository.update_profile(profile)
            if (
                profile_old.status != ProfileStatusEnum.COMPLETED
                and profile_new.status == ProfileStatusEnum.COMPLETED
            ):
                await self._kafka_producer.send_update_profile(profile_new)
        except adapter_errors.ProfileNotFoundError as e:
            raise services_errors.ProfileNotFoundError(
